  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (msg.includes("429") || msg.includes("Too Many")) {
        const delay = 5000 * (attempt + 1); // conservative
        await sleep(delay);
        continue;
      }
...
async function uploadCircuitManual(name: string) {
  ...
  // Arcium build puts them in build/${name}.arcis
  ...
  for (const idx of missing) {
    ...
    if (uploaded % 10 === 0 || uploaded === missing.length) {
      console.log(`  Progress: ${uploaded}/${missing.length} chunks uploaded`);
    }
    // Small delay between successful chunks
    await sleep(200);
  }

  console.log(`  All chunks present — finalizing...`);
...
async function main() {
  const names = ["init_tally", "add_vote", "reveal_tally"];
  ...
  for (const name of names) {
    try {
        ...
        console.error(`Error uploading ${name}: ${e.message}`);
    }
  }
  console.log("\nAll done!");
}
